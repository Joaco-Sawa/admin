...
interface CreateCommunicationModalTransversalProps {
  isOpen?: boolean;
  onClose: () => void;
}

export function CreateCommunicationModalTransversal({ isOpen = true, onClose }: CreateCommunicationModalTransversalProps) {
  const navigate = useNavigate();
  
  if (!isOpen) return null;

  const handleSelectType = (type: 'noticia' | 'popup') => {
    navigate(`/comunicaciones-transversales/crear?tipo=${type === 'noticia' ? 'Noticia' : 'Pop-up'}`);
    onClose();
  };
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50" style={{ backgroundColor: 'rgba(0, 0, 0, 0.4)' }}>
      <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-4xl w-full mx-4">
        ...
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-gray-900">Crear Nueva Comunicación Transversal</h2>
          <button
            onClick={onClose}
