...
export function CloneNoticia() {
  const navigate = useNavigate();
  const { programId, noticiaId } = useParams();
  const { showSuccess, showInfo, showError } = useToast();
  const { getComunicacion, addComunicacion } = usePrograms();
  
  // Obtener la comunicación original
  const comunicacionOriginal = getComunicacion(noticiaId || '');
  ...
  // Estados paso 2 - SIEMPRE empiezan vacíos en clonación
  ...
  // Cargar datos de la comunicación original al montar
  useEffect(() => {
    if (!comunicacionOriginal) {
      showError('No se puede clonar: la noticia original no existe');
      navigate(`/programa/${programId}/comunicaciones/muro`);
      return;
    }

    // Precargar título con prefijo "(Copia) " y truncar si excede 70 caracteres
    const prefijo = '(Copia) ';
    const tituloOriginal = comunicacionOriginal.titulo;
    let tituloClonado = prefijo + tituloOriginal;
    
    if (tituloClonado.length > 70) {
      // Truncar desde el final
      tituloClonado = tituloClonado.substring(0, 70);
    }
    
    setTitulo(tituloClonado);
    
    // Cargar contenido
    if (comunicacionOriginal.cuerpo && editor) {
      editor.commands.setContent(comunicacionOriginal.cuerpo);
    }
    
    // Cargar CTA
    if (comunicacionOriginal.cta) {
      setShowCTA(true);
      setCtaText(comunicacionOriginal.cta.texto);
      setCtaLink(comunicacionOriginal.cta.enlace);
    }
    
    // Cargar imagen y sus configuraciones
    if (comunicacionOriginal.imagen) {
      setUploadedImage(comunicacionOriginal.imagen.url);
      setImageFormat(comunicacionOriginal.imagen.formato);
      
      if (comunicacionOriginal.imagen.crop) {
        setCroppedAreaPixels(comunicacionOriginal.imagen.crop);
        // Crear imagen de previsualización con el crop original
        createCroppedImage(comunicacionOriginal.imagen.url, comunicacionOriginal.imagen.crop).then(croppedImg => {
          setPreviewImage(croppedImg);
        });
      }
    }
    
    // NO cargar fechas ni segmentación - siempre empiezan vacías en clonación
    // El estado siempre será Borrador para una noticia clonada
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [comunicacionOriginal, editor]);
  ...
  const handleSaveDraft = () => {
    ...
    // SIEMPRE estado Borrador para clonación
    const comunicacion = {
  ...
  const confirmPublish = () => {
    ...
    addComunicacion(comunicacion);
    showSuccess('¡Noticia clonada con éxito!');
    navigate(`/programa/${programId}/comunicaciones/muro`);
  };
  ...
  // Si no existe la comunicación original, no renderizar nada (el useEffect redireccionará)
  if (!comunicacionOriginal) {
    return (
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex-1 overflow-y-auto">
          ...
          <div className="bg-white px-8 pt-10 pb-6 border-b border-gray-200">
            ...
            <h1 className="text-2xl font-medium">Clonar Noticia</h1>
          </div>
