...
export function BannersEditar() {
  const navigate = useNavigate();
  const { bannerId } = useParams();
  const { showSuccess } = useToast();
  const { getBanner, updateBanner } = usePrograms();
  ...
  // Cargar datos del banner al montar
  useEffect(() => {
    if (!bannerId) return;
    
    const banner = getBanner(bannerId);
    if (banner) {
      setNombre(banner.nombre);
      setUrl(banner.url);
      
      // Parsear fecha y hora de inicio del formato "DD-MM-YYYY, HH:MM"
      if (banner.inicio) {
        const [fechaPart, horaPart] = banner.inicio.split(', ');
        const [day, month, year] = fechaPart.split('-');
        setFechaInicio(`${year}-${month}-${day}`);
        setHoraInicio(horaPart);
      }
      
      // Parsear fecha y hora de deshabilitación
      if (banner.deshabilitar) {
        const [fechaPart, horaPart] = banner.deshabilitar.split(', ');
        const [day, month, year] = fechaPart.split('-');
        setFechaDeshabilitar(`${year}-${month}-${day}`);
        setHoraDeshabilitar(horaPart);
      }
      
      setHabilitado(banner.habilitado);
      setBannerImage(banner.imagen || null);
    }
  }, [bannerId, getBanner]);
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex-1 overflow-y-auto p-8">
          <div className="max-w-4xl mx-auto">
            <div className="mb-6">
              <h1 className="text-2xl">Editar Banner</h1>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm p-8 space-y-6">
              ...
              <div className="flex gap-4 pt-6">
                ...
                <button 
                  onClick={handleActualizarBanner}
