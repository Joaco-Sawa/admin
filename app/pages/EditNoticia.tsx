...
export function EditNoticia() {
  const navigate = useNavigate();
  const { programId, noticiaId } = useParams();
  const { showSuccess, showInfo, showError } = useToast();
  const { getComunicacion, updateComunicacion } = usePrograms();
  
  // Obtener la comunicación existente
  const comunicacion = getComunicacion(noticiaId || '');
  ...
  useEffect(() => {
    if (comunicacion) {
      ...
      if (comunicacion.cuerpo && editor) {
        editor.commands.setContent(comunicacion.cuerpo);
      }
      if (comunicacion.cta) {
        setShowCTA(true);
        setCtaText(comunicacion.cta.texto);
        setCtaLink(comunicacion.cta.enlace);
      }
      if (comunicacion.imagen) {
        setUploadedImage(comunicacion.imagen.url);
        setImageFormat(comunicacion.imagen.formato);
        if (comunicacion.imagen.crop) {
          setCroppedAreaPixels(comunicacion.imagen.crop);
          // Generar imagen de previsualización con el crop original
          createCroppedImage(comunicacion.imagen.url, comunicacion.imagen.crop).then(croppedImg => {
            setPreviewImage(croppedImg);
          });
        }
      }
      if (comunicacion.fechaPublicacion) {
        if (comunicacion.estado === 'Borrador' || comunicacion.estado === 'Programada') {
          setPublishStrategy('scheduled');
          setScheduledDate(comunicacion.fechaPublicacion);
        }
      }
      if (comunicacion.despublicacionAuto && comunicacion.fechaDespublicacion) {
        setAutoUnpublish(true);
      ...
      if (comunicacion.segmentacion) {
        ...
        if (comunicacion.segmentacion.usuarios) {
          setCsvSummary({
      ...
      showError('No se pueden guardar los cambios: la noticia ya no existe');
      navigate(`/programa/${programId}/comunicaciones/muro`);
    }
  }, [comunicacion, editor, noticiaId]);
  ...
  const handleSaveDraft = () => {
    ...
    const updatedComunicacion = {
      ...
      segmentacion: undefined,
    };
    
    updateComunicacion(noticiaId, updatedComunicacion);
  ...
  const confirmPublish = () => {
    ...
    // Determinar estado basado en estrategia de publicación y fechas
    const now = new Date();
    const publishDate = publishStrategy === 'now'
      ? now.toISOString().split('T')[0]
      : scheduledDate;
    
    let estado: 'Borrador' | 'Programada' | 'Publicada' | 'Despublicada' = 'Publicada';
    ...
    // Verificar despublicación automática
    if (autoUnpublish && unpublishDate && unpublishTime) {
      const unpublishDateTime = new Date(`${unpublishDate}T${unpublishTime}`);
      if (unpublishDateTime <= now) {
        estado = 'Despublicada';
      }
    }
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex-1 overflow-y-auto">
          ...
          <div className="bg-white px-8 pt-10 pb-6 border-b border-gray-200">
            ...
            <h1 className="text-2xl font-medium">Editar Noticia</h1>
          </div>
